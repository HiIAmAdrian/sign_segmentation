...
LANDMARKS_CSV_DIR = Path("./output_landmarks_csv_p1_occlusion_normalised")
BLENDSHAPES_CSV_DIR = Path("./output_blendshapes_csv_p1")

ORIGINAL_BAG_FILES_DIR = Path("D:/SegmentationThesis/output_realsense60fps+tesla p1")


OUTPUT_FACIAL_PKL_DIR = Path("./final_facial_data_processed_p1")
        ...
        try:
            ...
            if not df_blendshapes_long.empty:
                if 'frame_id' not in df_blendshapes_long.columns:
                    print(f"    Warning: 'frame_id' missing in {bs_csv_path.name}."); df_blendshapes_long['frame_id'] = -1
