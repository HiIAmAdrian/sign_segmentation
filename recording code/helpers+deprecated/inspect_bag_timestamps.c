...
int main(int argc, char* argv[]) {
    ...
    rs2_config_enable_device_from_file_repeat_option(config, bag_filename, 0, &e);
    ...
    if (e || !profile) {  return EXIT_FAILURE; }

    playback_dev = rs2_pipeline_profile_get_device(profile, &e);
    check_rs_error(e, "Get device from profile");
    if (!rs2_is_device_extendable_to(playback_dev, RS2_EXTENSION_PLAYBACK, &e)) {  exit(EXIT_FAILURE); }
    check_rs_error(e, "Check playback extension");
    rs2_playback_device_set_real_time(playback_dev, 0, &e);
    ...
    while (true) {
        ...
        rs2_frame* depth_frame_ref = NULL;
        rs2_frame* color_frame_ref = NULL;
        ...
        if (rs2_is_frame_extendable_to(frames, RS2_EXTENSION_COMPOSITE_FRAME, &e)) {
            ...
            for (int i = 0; i < frame_count_in_set; ++i) {
                rs2_frame* single_frame = rs2_extract_frame(frames, i, &e);
                ...
                if (stream_type_local == RS2_STREAM_DEPTH && !depth_frame_ref) {
                    depth_frame_ref = single_frame;
                    rs2_frame_add_ref(depth_frame_ref, &e);
                    check_rs_error(e, "add ref depth composite");
                } else if (stream_type_local == RS2_STREAM_COLOR && !color_frame_ref) {
                    color_frame_ref = single_frame;
                    rs2_frame_add_ref(color_frame_ref, &e);
                    check_rs_error(e, "add ref color composite");
                }
                rs2_release_frame(single_frame);
            }
        } else {
        ...
        if (depth_frame_ref) {
            ...
            rs2_release_frame(depth_frame_ref);
        }
        ...
        if (color_frame_ref) {
            ...
            last_color_ts = current_color_ts;
             rs2_release_frame(color_frame_ref);
        }
